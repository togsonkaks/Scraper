...
(function () {
  ...
  // Smart cross-image quality upgrade system
  function smartUpgradeImageGroup(enrichedUrls) {
    debug('🧠 SMART UPGRADE: Analyzing image groups for quality optimization...');
    
    if (enrichedUrls.length < 2) {
      // Not enough images to group, just apply normal CDN upgrades
      return enrichedUrls.map(enriched => ({
        ...enriched,
        url: upgradeCDNUrl(enriched.url)
      }));
    }
    
    // Group images by similar base structure
    const groups = new Map();
    
    enrichedUrls.forEach((enriched, index) => {
      const url = enriched.url;
      if (!url) return;
      
      // Extract base structure (domain + path + product code pattern)
      let baseKey = url;
      
      // Remove quality parameters to find base structure
      baseKey = baseKey.replace(/[?&](?:w|width|imwidth|imageWidth)=\d+/gi, '');
      baseKey = baseKey.replace(/[?&](?:h|height|imheight|imageHeight)=\d+/gi, '');
      baseKey = baseKey.replace(/[?&](?:q|quality|q_auto)=[^&]*/gi, '');
      baseKey = baseKey.replace(/[?&](?:f|format|fm)=[^&]*/gi, '');
      baseKey = baseKey.replace(/[?&](?:dpr)=[\d.]+/gi, '');
      baseKey = baseKey.replace(/[?&]ts=\d+/gi, ''); // Remove timestamps
      baseKey = baseKey.replace(/_\d+x\d*/gi, ''); // Remove Shopify dimensions
      baseKey = baseKey.replace(/\$[^$]*\$/gi, ''); // Remove Scene7 transforms
      baseKey = baseKey.replace(/\?i10c=img\.resize[^)]*\)/gi, ''); // Remove Shocho resize
      ...
      groups.get(baseKey).push({ ...enriched, originalIndex: index });
    });
    
    debug(`🧠 SMART UPGRADE: Found ${groups.size} image groups`);
    
    // Process each group to find best quality and safely upgrade others
    const upgradedUrls = [];
    
    groups.forEach((imageGroup, baseKey) => {
      if (imageGroup.length < 2) {
        // Single image, just apply normal CDN upgrade
        upgradedUrls.push(...imageGroup.map(img => ({
          ...img,
          url: upgradeCDNUrl(img.url)
        })));
        return;
      }
      
      debug(`🧠 ANALYZING GROUP: ${imageGroup.length} similar images from base: ${baseKey.substring(baseKey.lastIndexOf('/') + 1)}`);
      
      // Score all images in the group using existing scoring system
      const scoredImages = imageGroup.map(img => {
        const upgradedUrl = upgradeCDNUrl(img.url);
        const currentScore = scoreImageURL(img.url, img.element, img.originalIndex);
        const upgradedScore = scoreImageURL(upgradedUrl, img.element, img.originalIndex);
        ...
        return {
          ...
          upgradedScore: upgradedScore,
          bestScore: Math.max(currentScore, upgradedScore)
        };
      });
      
      // Find the baseline (highest scoring image)
      const baseline = scoredImages.reduce((best, current) => 
        current.bestScore > best.bestScore ? current : best
      );
      
      debug(`🏆 GROUP BASELINE: Score ${baseline.bestScore} from ${baseline.url.substring(baseline.url.lastIndexOf('/') + 1)}`);
      
      if (baseline.bestScore <= 50) {
        // No good quality found in group, use normal upgrades
        upgradedUrls.push(...scoredImages.map(img => ({
          ...img,
          url: img.upgradedUrl
        })));
        return;
      ...
      // Extract quality parameters from baseline image
      const baselineUrl = baseline.bestScore === baseline.upgradedScore ? baseline.upgradedUrl : baseline.url;
      const bestParams = extractQualityParams(baselineUrl);
      
      debug(`🎯 BASELINE PARAMS:`, bestParams);
      
      // Apply smart upgrades to other images in group
      scoredImages.forEach(img => {
        if (img === baseline) {
          // Keep baseline image unchanged (it's already the best)
          upgradedUrls.push({
            ...img,
            url: img.bestScore === img.upgradedScore ? img.upgradedUrl : img.url
          });
          return;
        }
        
        // For non-baseline images, try applying baseline parameters
        const smartUpgraded = applyQualityParams(img.upgradedUrl, bestParams);
        const smartScore = scoreImageURL(smartUpgraded, img.element, img.originalIndex);
        
        // Only upgrade if it improves the score significantly
        const improvementThreshold = Math.max(5, baseline.bestScore * 0.05); // 5% improvement or 5 points
        
        if (smartScore > img.bestScore + improvementThreshold) {
          debug(`✨ SMART UPGRADED: ${img.url.substring(img.url.lastIndexOf('/') + 1)} (${img.bestScore} → ${smartScore})`);
          upgradedUrls.push({ ...img, url: smartUpgraded });
        } else {
          // Not worth upgrading, keep current best
          upgradedUrls.push({
            ...img,
            url: img.bestScore === img.upgradedScore ? img.upgradedUrl : img.url
          });
    ...
    upgradedUrls.sort((a, b) => a.originalIndex - b.originalIndex);
    
    debug(`🧠 SMART UPGRADE: Processed ${upgradedUrls.length} images with group intelligence`);
    return upgradedUrls;
  }
  
  // Extract quality parameters from URL
  function extractQualityParams(url) {
    const params = {};
    
    // Width parameters (Zara, Guess, etc.)
    const widthMatch = url.match(/[?&](?:w|width|imwidth|imageWidth)=(\d+)/i);
    if (widthMatch) params.width = parseInt(widthMatch[1]);
    
    // Height parameters  
    const heightMatch = url.match(/[?&](?:h|height|imheight|imageHeight)=(\d+)/i);
    if (heightMatch) params.height = parseInt(heightMatch[1]);
    
    // Quality parameters (Cloudinary, etc.)
    const qualityMatch = url.match(/[?&](?:q|quality|q_auto)=([^&]*)/i);
    if (qualityMatch) params.quality = qualityMatch[1];
    
    // Format parameters
    const formatMatch = url.match(/[?&](?:f|format|fm)=([^&]*)/i);
    if (formatMatch) params.format = formatMatch[1];
    
    // DPR parameters (device pixel ratio)
    const dprMatch = url.match(/[?&]dpr[_=]([\d.]+)/i);
    if (dprMatch) params.dpr = parseFloat(dprMatch[1]);
    
    // Shopify dimensions
    const shopifyMatch = url.match(/_(\d+)x(\d*)/);
    if (shopifyMatch) {
      params.shopifyWidth = parseInt(shopifyMatch[1]);
      if (shopifyMatch[2]) params.shopifyHeight = parseInt(shopifyMatch[2]);
    }
    
    // Scene7 transforms (Free People, Urban Outfitters)
    const scene7Match = url.match(/\$([^$]+)\$/);
    if (scene7Match) params.scene7Transform = scene7Match[1];
    
    return params;
  }
  
  // Apply quality parameters to URL (conservative approach)
  function applyQualityParams(url, bestParams) {
    let upgraded = url;
    
    // Only apply if we have meaningful parameters to apply
    if (Object.keys(bestParams).length === 0) {
      return upgraded;
    }
    
    // Apply width if target has lower or no width (and bestParams has width)
    if (bestParams.width && bestParams.width > 400) { // Only apply reasonable widths
      const currentWidthMatch = upgraded.match(/[?&](?:w|width|imwidth|imageWidth)=(\d+)/i);
      const currentWidth = currentWidthMatch ? parseInt(currentWidthMatch[1]) : 0;
      
      if (currentWidth === 0 || currentWidth < bestParams.width) {
        if (currentWidthMatch) {
          upgraded = upgraded.replace(/([?&])(?:w|width|imwidth|imageWidth)=\d+/i, `$1w=${bestParams.width}`);
        } else {
          const separator = upgraded.includes('?') ? '&' : '?';
          upgraded += `${separator}w=${bestParams.width}`;
        }
      }
    }
    
    // Apply high-quality format if target doesn't have format
    if (bestParams.format && /^(auto|webp|avif)$/i.test(bestParams.format)) {
      const currentFormatMatch = upgraded.match(/[?&](?:f|format|fm)=([^&]*)/i);
      if (!currentFormatMatch) {
        const separator = upgraded.includes('?') ? '&' : '?';
        upgraded += `${separator}f=${bestParams.format}`;
      }
    }
    
    // Apply quality if target doesn't have quality (and it's a high quality setting)
    if (bestParams.quality && /(auto|best|90|95|100)/i.test(bestParams.quality)) {
      const currentQualityMatch = upgraded.match(/[?&](?:q|quality|q_auto)=([^&]*)/i);
      if (!currentQualityMatch) {
        const separator = upgraded.includes('?') ? '&' : '?';
        upgraded += `${separator}q=${bestParams.quality}`;
      }
    }
    
    // Apply DPR if target has lower or no DPR (and it's reasonable)
    if (bestParams.dpr && bestParams.dpr >= 1.5 && bestParams.dpr <= 3) {
      const currentDprMatch = upgraded.match(/[?&]dpr[_=]([\d.]+)/i);
      const currentDpr = currentDprMatch ? parseFloat(currentDprMatch[1]) : 1;
      
      if (currentDpr < bestParams.dpr) {
        if (currentDprMatch) {
          upgraded = upgraded.replace(/([?&])dpr[_=][\d.]+/i, `$1dpr=${bestParams.dpr}`);
        } else {
          const separator = upgraded.includes('?') ? '&' : '?';
          upgraded += `${separator}dpr=${bestParams.dpr}`;
        }
      }
    }
    
    // Apply Shopify dimensions if target has smaller dimensions
    if (bestParams.shopifyWidth && bestParams.shopifyWidth >= 1000) {
      const currentShopifyMatch = upgraded.match(/_(\d+)x(\d*)/);
      const currentShopifyWidth = currentShopifyMatch ? parseInt(currentShopifyMatch[1]) : 0;
      
      if (currentShopifyWidth > 0 && currentShopifyWidth < bestParams.shopifyWidth) {
        const heightPart = bestParams.shopifyHeight ? bestParams.shopifyHeight : (currentShopifyMatch[2] || '');
        upgraded = upgraded.replace(/_\d+x\d*/, `_${bestParams.shopifyWidth}x${heightPart}`);
      }
    }
    
    // Apply Scene7 transforms if target has lower quality transform
    if (bestParams.scene7Transform && isHighQualityScene7Transform(bestParams.scene7Transform)) {
      const currentScene7Match = upgraded.match(/\$([^$]+)\$/);
      if (!currentScene7Match || isLowerQualityTransform(currentScene7Match[1], bestParams.scene7Transform)) {
        if (currentScene7Match) {
          upgraded = upgraded.replace(/\$[^$]+\$/, `$${bestParams.scene7Transform}$`);
        } else {
          // Add transform before file extension
          upgraded = upgraded.replace(/(\.[^.?]+)(\?|$)/, `$${bestParams.scene7Transform}$$1$2`);
        }
      }
    }
  ...
  // Check if Scene7 transform is high quality
  function isHighQualityScene7Transform(transform) {
    const highQualityTransforms = ['redesign-zoom-5x', 'zoom-5x', 'large', 'xlarge'];
    return highQualityTransforms.some(t => transform.includes(t));
  }
  
  // Check if current Scene7 transform is lower quality than best
  function isLowerQualityTransform(current, best) {
    const qualityRanking = {
      'redesign-zoom-5x': 10,
      'zoom-5x': 9,
      'xlarge': 8,
      'large': 7,
      'a15-pdp-detail-shot': 5,
      'pdp-detail-shot': 4,
      'medium': 3,
      'small': 2,
      'default': 1
    ...
    const currentRank = qualityRanking[current] || 1;
    const bestRank = qualityRanking[best] || 1;
