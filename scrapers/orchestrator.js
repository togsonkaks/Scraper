...
(function () {
  ...
  async function getImagesGeneric() {
    ...
    // GENERIC ZOOM/MODAL DETECTION (common across e-commerce sites)
    debug('🔍 Trying generic zoom/modal detection...');
    const zoomImages = [];
    
    // Helper to normalize URLs (handle relative, protocol-relative, absolute)
    ...
    // 1. DATA ATTRIBUTE ZOOM DETECTION (img, anchor, div elements)
    const zoomDataSelectors = [
      '[data-zoom-image]', '[data-large-image]', '[data-hires]', 
      '[data-full]', '[data-zoom]', '[data-fancybox]', '[data-photoswipe]',
      '[data-zoom-src]', '[data-large-src]', '[data-magnify]'
    ];
    
    for (const sel of zoomDataSelectors) {
      document.querySelectorAll(sel).forEach(el => {
        const zoomUrl = el.getAttribute('data-zoom-image') || 
                       ...
                       el.getAttribute('data-photoswipe') ||
                       el.getAttribute('data-zoom-src') ||
                       el.getAttribute('data-large-src') ||
                       el.getAttribute('data-magnify');
        const normalizedUrl = zoomUrl ? normalizeUrl(zoomUrl) : null;
        if (normalizedUrl) {
          zoomImages.push(normalizedUrl);
          debug('🔍 Generic zoom data found:', normalizedUrl);
        }
      });
    }
    
    // 2. SRCSET LARGEST CANDIDATE DETECTION
    document.querySelectorAll('img[srcset]').forEach(img => {
      ...
      if (srcset) {
        ...
        candidates.forEach(candidate => {
          ...
          // Extract width from descriptor (e.g., "2000w", "2x")
          let width = 0;
          if (descriptor) {
            if (descriptor.endsWith('w')) {
              ...
              width = Math.max(multiplier * 1000, 800); // Conservative estimate for density descriptors
            }
          }
        ...
        // REMOVED hard threshold - let scorer decide quality
        if (largest) {
          const normalizedUrl = normalizeUrl(largest);
          if (normalizedUrl) {
            zoomImages.push(normalizedUrl);
            debug('🔍 Generic srcset zoom found:', normalizedUrl, `(${maxWidth}px)`);
          }
        }
      }
    });
    
    // 3. PICTURE/SOURCE ELEMENT DETECTION (responsive high-res)
    document.querySelectorAll('picture source').forEach(source => {
      const srcset = source.getAttribute('srcset');
      const media = source.getAttribute('media');
      
      // Target larger screen media queries for high-res images
      if (srcset && (!media || /min-width.*[8-9]\d{2}|[1-9]\d{3}/.test(media))) {
        const urls = srcset.split(',').map(s => s.trim().split(/\s+/)[0]);
        urls.forEach(url => {
          const normalizedUrl = normalizeUrl(url);
          if (normalizedUrl) {
            zoomImages.push(normalizedUrl);
            debug('🔍 Generic picture/source zoom found:', normalizedUrl);
          }
        });
      }
    });
    
    // 4. LIGHTBOX/MODAL LINK DETECTION (case-insensitive, broader anchor support)
    document.querySelectorAll('a[href$=".jpg" i], a[href$=".jpeg" i], a[href$=".png" i], a[href$=".webp" i]').forEach(link => {
      const href = link.getAttribute('href');
      
      // Heuristic: Large image links (with or without nested img)
      if (href && (/\d{4}|large|zoom|full|hd|hi-?res/i.test(href) || link.querySelector('img'))) {
        const normalizedUrl = normalizeUrl(href);
        if (normalizedUrl) {
          zoomImages.push(normalizedUrl);
          debug('🔍 Generic lightbox zoom found:', normalizedUrl);
        }
      }
    });
    
    // 5. SIZE HEURISTIC DETECTION (URLs with size hints)
    document.querySelectorAll('img[src]').forEach(img => {
      const src = img.currentSrc || img.src;
      // Look for size patterns indicating high-resolution
      if (src && /(?:w=|wid=|width=|size=)\d{3,}|\d{4}x\d{3}|\d{4}px|[_\-](?:xl|xxl|large|huge|full|hd|[1-9]\d{3})[\._\-]/i.test(src)) {
        const normalizedUrl = normalizeUrl(src);
        if (normalizedUrl) {
          zoomImages.push(normalizedUrl);
          debug('🔍 Generic size heuristic zoom found:', normalizedUrl);
        }
      }
    });
    
    // INTEGRATE WITH EXISTING PIPELINE: Process through upgradeCDNUrl, filters, scoring
    if (zoomImages.length > 0) {
      debug(`🔍 Processing ${zoomImages.length} raw zoom images through pipeline...`);
      
      // Remove duplicates and process through existing pipeline
      const rawZoomUrls = [...new Set(zoomImages)];
      let processedZoomUrls = [];
      
      for (const url of rawZoomUrls) {
        // Apply CDN upgrades
        const upgradedUrl = upgradeCDNUrl(url);
        
        // Apply junk filtering
        if (!JUNK_IMG.test(upgradedUrl)) {
          // Apply Shopify file filtering if applicable
          if (shouldBlockShopifyFiles && shouldBlockShopifyFiles(upgradedUrl)) {
            debug('🔍 Shopify filter blocked:', upgradedUrl);
            continue;
          }
          
          processedZoomUrls.push(upgradedUrl);
        }
      }
      
      // If we have good zoom images, process through scoring and return
      if (processedZoomUrls.length >= 3) {
        const scoredZoomImages = await uniqueImages(processedZoomUrls);
        debug(`✅ Generic zoom success: ${scoredZoomImages.length} high-quality zoom images found`);
        mark('images', { selectors: ['zoom-detection'], attr: 'various', method: 'generic-zoom', urls: scoredZoomImages.slice(0, 30) });
        return scoredZoomImages.slice(0, 30);
      } else {
        // Merge with gallery results (prioritize zoom images)
        debug(`🔍 Generic zoom partial: ${processedZoomUrls.length} zoom images found, merging with gallery...`);
        // Continue to gallery selectors but prepend zoom images
      }
    }
