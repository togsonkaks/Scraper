...
ipcMain.handle('validate-selectors', async (_evt, urlOrHost) => {
  ...
  if (!savedSelectors || Object.keys(savedSelectors).filter(k => k !== '__history').length === 0) {
    return {
      savedSelectors: {},
      testResults: {}
    };
  ...
  // Test each saved field with simple, direct validation
